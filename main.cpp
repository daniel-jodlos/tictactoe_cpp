...
void print_help();
template <class T>
void print_help_on_fail(T value, std::function<bool(T)> predicate);
...
int main(int argc, char **args) {
  ...
  } else if (std::strcmp(args[1], "websocket") == 0) {
    do_setup_game_server(argc, args);
  } else {
    print_help();
    return -1;
  }
...
#define print_and_exit_if(predicate, msg)                                      \
  if (predicate) {                                                             \
    std::puts(msg);                                                            \
    std::exit(0);                                                              \
  }
...
void do_create_remote_game(int argc, char **args) {
  ...
  Game g = createRemoteGameServer<Player, RemotePlayer>(
      port, std::rand() % 2 == 0, b);
