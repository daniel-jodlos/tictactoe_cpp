...
void Player::onFinish(Element result) const {
  ...
  if (result == Empty)
    game_screen::talk_to_player(getPlaysWith(), "Draw");
  else
    game_screen::talk_to_player(getPlaysWith(),
                                result == getPlaysWith() ? "Winner" : "Looser");
}

Element Player::getPlaysWith() const { return _playsWith; }

void Player::onError(std::string error) const noexcept {}
